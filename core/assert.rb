...
module Assert
	...
	def self.setDebug(state)
		@debug = state
	end
	
	def self.setSoftAssert(state)
    @soft_assert = state
  end
	...
	def self.assertComparison(description, leftValue, rightValue, operation)
		@operation = operation
	...
	def self.assertBoolean(description, boolean, expected = " Expected true but was false")
		if(!boolean)
		  if(@soft_assert)
		    puts "Failed:: " + description + expected
		  else
	...
	def self.warnComparison(description, leftValue, rightValue, operation)
		@operation = operation
		boolean = compare(leftValue, rightValue)
		expected = " Expected:: [" + leftValue.to_s + "] " + @operation + " [" + rightValue.to_s + "]"
		if(!boolean) then
	...
	def self.warnBoolean(description, boolean, expected = " Expected true but was false")
		if(!boolean) then
	...
	def self.compare(leftValue, rightValue)
		comparison = false
		converted = false

		# if is for regular expression, all value should be string
	...
	private_class_method :assertComparison, :assertBoolean, :warnComparison, :warnBoolean, :compare
end
